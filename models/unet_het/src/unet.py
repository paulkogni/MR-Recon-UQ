...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # decode mu
        mu = self.up1_mu(x5, x4)
        mu = self.up2_mu(mu, x3)
        mu = self.up3_mu(mu, x2)
        mu = self.up4_mu(mu, x1)
        mu = self.out_mu(mu)

        # decode logvar
        logvar = self.up1_logvar(x5, x4)
        logvar = self.up2_logvar(logvar, x3)
        logvar = self.up3_logvar(logvar, x2)
    ...
    def make_prediction(self, img):
        ...
        with torch.no_grad():
            for i in range(20):
                output = torch.distributions.Normal(mu, var).sample()
        ...
        if save_model == True:
            if eval_running_loss < best_total_eval_loss:
                ...
                torch.save(
                    ...
                    f"{save_path}unet_het_best_eval_epoch{epoch}.pth",
                )
                ...
                if epoch % 50 == 0:
                    if ssim > best_ssim:
                        ...
                        torch.save(
                            ...
                            f"{save_path}unet_het_best_ssim_epoch{epoch}.pth",
                        )
