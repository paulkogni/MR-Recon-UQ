...
class SampleZBlock(nn.Module):
    ...
    def forward(self, pre_z):
        ...
        z = mu + sigma * torch.randn_like(sigma, dtype=torch.float32) # TODO change here to deterministic
...
class Likelihood(nn.Module):
    ...
    def forward(self, z):
        ...
        for i, block in enumerate(self.s_layer):
            ...
            s[-i-1] = torch.nn.functional.interpolate(s_in, size=[self.image_size[1], self.image_size[2]], mode='bilinear')# mode='nearest')
...
class PHIRec(nn.Module):
    ...
    def sample_posterior(self):
        ...
        for i, _ in enumerate(z_sample):
            z_sample[i] = mu[i] + sigma[i] * torch.randn_like(sigma[i]) # TODO change here to deterministic
    ...
    def residual_multinoulli_loss(self, reconstruction, target):
        ...
        for ii, s_ii in zip(reversed(range(self.latent_levels)),
            ...
            if ii == self.latent_levels-1:
                ...
                self.loss_dict['residual_multinoulli_loss_lvl%d' % ii] = criterion(self.s_accumulated[ii] + self.patch, target)
...
def train_model(model, train_loader, eval_loader, optim, epochs=1, save_model=None, save_path=None, continue_training_path=None, eval_metric=None, device='cuda:0'):
    """
    ...
    for epoch in range(end_epoch, epochs):
        ...
        train_running_loss, train_kl_loss, train_recon_loss = compute_train_loss_and_train(train_loader, model, optim, device, epoch=epoch)

        # compute evaluation loss
        eval_running_loss, eval_kl_loss, eval_recon_loss = compute_eval_loss(eval_loader, model, device, epoch)

        # compute GED between samples and GTs on evaluation set
        if eval_metric:
            if epoch % 50 == 0: # compute only every 50 epochs
                psnr, ssim, _ = utils.eval_ssim_psnr_ncc(model, eval_loader, model_type='phiseg', n_samples=10, device=device)
                print('psnr:',psnr)
                print('ssim:',ssim)
